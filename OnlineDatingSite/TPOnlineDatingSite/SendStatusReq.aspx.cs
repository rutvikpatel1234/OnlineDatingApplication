...
{
    public partial class SendStatusReq : System.Web.UI.Page
    {
        memberlikes req = new memberlikes();
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            username = Session["username"].ToString();
            DataSet CourseDataSet = req.TPGetRequestMember(username, value);
            ...
            if (CourseDataSet != null)
            {
                ...
                if (dataTable.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dataTable.Rows.Count; i++)
                    {
                        ...
                        sb.Append("<a href=\"MemberHomeView.aspx?_id=" + dataTable.Rows[i]["username"] + "\"class=\"btn btn-info btn-sm fas fa-eye fa-2x\"></a>" /*+ "<a href=\"MemberHomeView.aspx?_id=" + dataTable.Rows[i]["username"] + "\"class=\"btn btn-info btn-sm fas fa-paper-plane fa-2x\"></a>"*/
                            + "<a href=\"LikeDisLike.aspx?_userTo=" + dataTable.Rows[i]["username"] + "&value=0&Form=Req" + "\"class=\"btn btn-danger btn-sm fas fa-times fa-2x\"></a>");
                        sb.Append("</div></div>");
                        //}
                    }
